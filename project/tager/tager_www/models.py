...
#This method returns to the Seller (User) the list of buyers (User) interested in his (specific) post
    def getInterestedIn(self, post):       
        interested = InterestedIn.objects.filter(user_id_seller = self.id, post = post.id)  #this filters all records related to the seller and the current post
        x = []               #this initilizes a new list for the Buyer Ids to be placed
        for i in interested:   # this loops all the records in the model in order to return the desired buyer's IDs and place them in list X
